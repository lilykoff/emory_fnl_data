...
subtitle: "Using Functional Data Analysis" 
execute: 
  echo: false
  cache: true
  warning: false
...
[https://shorturl.at/AJOnk](https://shorturl.at/AJOnk)
...
# 
# svg(here::here("docs", "figs", "highlight.svg"), width = 12, height = 5)
# temp
# dev.off()
...
p2 = df %>%
  ...
  labs(x = "Acceleration (g)", y = "Lagged acceleration (g)") + 
  theme(panel.grid = element_blank())
# svg(here::here("docs", "figs", "2d1.svg"), width = 12, height = 5)
# p 
# dev.off()
# 
# svg(here::here("docs", "figs", "2d2.svg"), width = 12, height = 5)
# p2
# dev.off()
```
...
# svg(here::here("docs", "figs", "points.svg"), width = 12, height = 5)
# temp
# dev.off()
...
p
# svg(here::here("docs", "figs", "grid.svg"), width = 12, height = 5)
# p
# dev.off()
...
### Aside: functional regression approach {.scrollable}
...
+ acceleration matrix = $$\begin{bmatrix} v_1(1) & v_1(1) & v_1(1) & v_1(2) & v_1(2) & v_1(3) \\
v_2(1) & v_2(1) & v_2(1) & v_2(2) & v_2(2) & v_2(3) \\
\end{bmatrix}
$$ 

+ lag acceleration matrix = $$\begin{bmatrix} v_1(2) & v_1(3) & v_1(4) & v_1(3) & v_1(4) & v_1(4) \\
v_2(2) & v_2(3) & v_2(4) & v_2(3) & v_2(4) & v_2(4)  \\
\end{bmatrix}
$$ 

+ lag matrix = $$\begin{bmatrix} 1 & 2 & 3 & 1 & 2 & 1\\
1 & 2 & 3 & 1 & 2 & 1\\\end{bmatrix}
$$ 
...
write_csv(day1, here::here("docs", "data", "day1.csv.gz"))
```


```{r}
day1 = read_csv(here::here("docs", "data", "day1.csv.gz"))
p1 = day1 %>%
  mutate(vm = sqrt(X^2 + Y^2 + Z^2)) %>%
...
+ Linear scalar on function regression (SoFR) model:
$$\text{logit}(p_{ij}^{i_0}) =\beta_0^{i_0} + \int_S \beta_1(s) X_{ij}(s)ds + \epsilon_{ij}$$
Where $S \in \{1, 2, \dots, 432\}$, $X_{ij}(s)$ is number of points for subject $i$ in second $j$ in grid cell $s$, $\epsilon_{ij} \sim \mathcal{N}(0, \sigma^2)$

+ Nonlinear scalar on function regression (SoFR) model:
$$\text{logit}(p_{ij}^{i_0}) =\beta_0^{i_0} + \int_S f\big(\beta_1(s) X_{ij}(s)\big)ds + \epsilon_{ij}$$
